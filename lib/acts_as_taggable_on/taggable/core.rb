...
module ActsAsTaggableOn::Taggable
  module Core
    ...
    def process_dirty_object(context, new_list)
      ...
      if version >= "5.2."
        _changed_attributes = @attributes_changed_by_setter
      else
      ...
      if changed_attributes.include?(attrib)
        ...
        _changed_attributes.delete(attrib) if old.to_s == value.to_s
      else
        old = tag_list_on(context)
        if self.class.preserve_tag_order
          _changed_attributes[attrib] = old if old.to_s != value.to_s
        else
          _changed_attributes[attrib] = old.to_s if old.sort != ActsAsTaggableOn.default_parser.new(value).parse.sort
        end
      end
    end
